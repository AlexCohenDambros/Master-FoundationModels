...
# MAIN PIPELINE
# ======================================
def run_full_experiment_pipeline(experiment_name: str, path_trained_models: str = "trained_models_encoder", top_k:int = 2, use_noise = True):
    ...
    # ======================================
    def process_dataset(state_code, year, prediction_length):
        ...
        for i in range(n_series):
            ...
            model = NBEATSModel(
            input_chunk_length=train_cpu.shape[1] - prediction_length,
        ...
        for i in range(n_series):
            ...
            for t in range(tensor_train.shape[1] - prediction_length):
        ...
        # -----------------------------
        start = time.time()
        n_pts = train_cpu.shape[1]
        panel_df = pd.DataFrame({
            "unique_id": np.repeat(np.arange(n_series), n_pts),
            "ds": np.tile(np.arange(n_pts), n_series),
            "y": train_cpu.flatten(),
        })
        nf_pt = NeuralForecast(
            models=[NF_PatchTST(input_size=n_pts - prediction_length, h=prediction_length, max_steps=100, scaler_type="standard")],
            freq=1,
        )
        nf_pt.fit(panel_df)
        fc_pt = nf_pt.predict()
        ...
        output_patchtst = torch.tensor(
            fc_pt["PatchTST"].to_numpy().reshape(n_series, prediction_length),
        ...
        nf_it = NeuralForecast(
            models=[NF_iTransformer(input_size=n_pts - prediction_length, h=prediction_length, n_series=n_series, max_steps=100, scaler_type="standard")],
            freq=1,
        )
        nf_it.fit(panel_df)
        fc_it = nf_it.predict()
        ...
        output_itransformer = torch.tensor(
            fc_it["iTransformer"].to_numpy().reshape(n_series, prediction_length),
            dtype=torch.float32, device=device,
        )
        ...
        # Models used in the ensemble (exclude FM-MoE)
        ensemble_models = {
            k: v for k, v in model_outputs.items() if k != "FM-MoE"
        }

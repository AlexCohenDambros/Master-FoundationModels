...
import warnings
import math
from .configuration_foundation_moe import FoundationMoEConfig
...
class TimeSeriesInputEmbedding(nn.Module):
    """
    Applies a gated linear embedding to time series input.
    Combines a non-linear gate with a value projection.
...
class RotaryEmbedding(torch.nn.Module):
    """
    Rotary Positional Embedding (RoPE) for time series.
    Generates cached cosine and sine tensors used to rotate input vectors
    in attention layers, enabling position-aware representations without absolute encodings.

    Input: x shape [batch_size, num_heads, seq_len, head_dim]
    Output: (cos, sin) each with shape [seq_len, dim]
    """
...
class FoundationMoERMSNorm(nn.Module):
    """
    Root Mean Square Layer Normalization (RMSNorm).
    Normaliza cada vetor de características com sua média quadrática (RMS),
    sem subtrair a média como na LayerNorm tradicional.
...
class FoundationMoESparseExpertsLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        Returns: aggregated expert predictions (batch_size, horizon, output_dim)
        """

        # A tensor de entrada é achatada para (batch_size * sequence_length, hidden_dim).
        # Isso permite aplicar o roteamento token-wise, ou seja, roteia cada token individualmente para experts diferentes.
        batch_size, sequence_length, hidden_dim = hidden_states.shape
        hidden_states = hidden_states.view(-1, hidden_dim)

        # Compute router logits
        # Usa uma camada linear (self.gate) para gerar scores (logits) de roteamento.
        router_logits = self.gate(hidden_states) 

        # Aplica softmax nos logits para obter as probabilidades (distribuição) de seleção para cada expert.
        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)

        # Select top-k experts per token
        # Seleciona os top_k especialistas com maiores probabilidades para cada token.
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=1)  
        # Converte as probabilidades de volta ao mesmo dtype dos hidden_states (por ex., bfloat16 ou float32).
        routing_weights = routing_weights.to(hidden_states.dtype)


        # Cria um tensor vazio que vai acumular a saída ponderada dos especialistas.
        ...
        # Converte os índices dos especialistas selecionados para vetores one-hot.
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0)

        # Itera sobre todos os especialistas do modelo.
        for expert_idx in range(self.num_experts):
            # Seleciona o especialista atual.
            ...
            # Executa o especialista atual só nos tokens que o escolheram.
            # Multiplica a saída pela probabilidade de roteamento respectiva.
            current_hidden_states = expert_layer(current_state) * routing_weights[top_x, idx, None]

            # Adiciona as contribuições do especialista para os tokens corretos no final_hidden_states.
            final_hidden_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))

        # Retorna ao shape original para seguir no pipeline.
        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)

        # Retorna a saída dos especialistas e os logits de roteamento
...
class MoeAttention(nn.Module):
    ...
    def __init__(self, config: FoundationMoEConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class MoeDecoderLayer(nn.Module):
    def __init__(self, config: FoundationMoEConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.ffn_layer = FoundationMoESparseExpertsLayer(config)
        self.input_layernorm = FoundationMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = FoundationMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class MoePreTrainedModel(PreTrainedModel):
    config_class = FoundationMoEConfig
...
class Model(MoePreTrainedModel):
    """
    ...
    def __init__(self, config: FoundationMoEConfig):
        ...
        self.norm = FoundationMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
